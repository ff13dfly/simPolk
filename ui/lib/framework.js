;(function(){
	...
	const self={
		...
		regComponent:function(r){
			...
			//挂载方法到root下
			const hks=r.hooks,lps=r.loops;
			if(hks && hks.length>0)for(let i in hks) window[me.appName][hks[i]]=fun[hks[i]];
			if(lps && lps.length>0)for(let i in lps) window[me.appName][lps[i]]=fun[lps[i]];
		...
		getConfig:function(chain){
			...
			if(self.type(chain)=='array'){
				const rst=false;
				for(let i in chain)if(me[chain[i]]){rst=me[chain[i]] }else{return false} 
				return rst;
			}
			return me;
		},
		...
		checkTimer:function(){
			...
			const tms=self.getMemory([me.memKeys.timer]),after={};
		...
		struct:function(p){
			...
			for(let k in cps){
				if(!self.empty(cps[k].hooks)){
					const hks=cps[k].hooks,len=hks.length;
		...
		jsonp:function(server,cfg,ck){
			...
			if(cfg.param!=undefined) for(let k in cfg.param) furl+='&'+k+'='+cfg.param[k];
			furl+='&callback=?'
		...
		queueRemove:function(id,k){
			const chain=[me.memKeys.queue,k];
			let arr=self.getMemory(chain),narr=[];
			for(let i in arr)if(i==id) arr[i]=null;
			for(let i in arr){
		...
		queuePop:function(k){
			const chain=[me.memKeys.queue,k],arr=self.getMemory(chain);
			return arr.pop();
		},
		...
		getUrlXY:function(){const h=window.location.hash,rst=[];const b=h.substring(1,h.length).split('-');for(let i=0,len=b.length;i<len;i++)rst.push(parseInt(b[i]));return rst;},
		getID:function(s){const a=s.split('_');return a[a.length-1]},
		analysisUrl:function(){
			let arr=location.pathname.split('/');
			arr.pop();
			let svc=location.host,pt='/';
       	...
       	chr:function(n,pre){n=n||7;pre=pre||'';for(let i=0;i<n;i++)pre+=i%2?String.fromCharCode(self.rand(65,90)):String.fromCharCode(self.rand(97,122));return pre},
